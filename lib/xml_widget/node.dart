...
class NodeData {
    ...
    if (_rawChildren == null) {
      ...
      if (_template.node is xml.XmlElement) {
        ...
        for (var childTemplate in _template.children) {
          try {
            _rawChildren!.addAll(childTemplate.generate(status, control, message) as Iterable<NodeData>);
          } catch (e) {
    ...
    if (_children == null) {
      ...
      if (_template.node is xml.XmlElement) {
        ...
        for (var child in rawChildren) {
          if (child.name?.prefix == "arg") {
            for (var sub in child.rawChildren) {
              _setNode(child.name.toString(), sub);
            }
          } else if ((child.name!= null && child.name!.local.isNotEmpty && RegExp(r"^[a-z]").hasMatch(child.name!.local)) || child.name?.prefix == "attr") {
            for (var sub in child.rawChildren) {
  ...
  dynamic element() {
    if (isElement) {
      ...
      _ItemInfo? info =
      MayaXmlWidget._constructors[element.name.toString().toLowerCase()];
      if (info != null) {
      ...
      default:
        {
          ...
          _ItemInfo? info = MayaXmlWidget._constructors[T];
          if (info == null) {
            return control.onUnkown?.call(this, _getKey());
          } else {
