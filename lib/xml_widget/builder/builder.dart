...
/// Build a [.xml.dart] file
class MayaXmlWidgetBuilder extends Builder {
  final BuilderOptions options;

  MayaXmlWidgetBuilder(this.options);

  @override
  FutureOr<void> build(BuildStep buildStep) async {
    BuilderStatus status = BuilderStatus();
    status.imports.add("package:flutter_maya_sdk/xml_widget/ui.dart");
    status.imports.add("package:flutter_maya_sdk/xml_widget/register.dart");
    ...
    await buildStep.writeAsString(
        buildStep.inputId.changeExtension('.xml.dart'),
        output
    );
  }
  ...
  void _processType(BuilderStatus status, ClassElement classElement) {
    ...
    if (classElement.isDartCoreEnum) {
      codes.add('MayaXmlWidget.registerEnum(${classElement.name}.values);');
    } else {
        ...
        for (var con in classElement.constructors) {
          if (con.isPublic) {
            ...
            if (_isConstructorInline(con)) {
              List<String> segs = [];
              segs.add('MayaXmlWidget.registerInline(${classElement.name}, "${con.name}", false, (node, method) {');
              if (con.name.isEmpty) {
              ...
              List<String> segs = [];
              segs.add('MayaXmlWidget.register("$constructorName", (node, key) {');
      ...
      for (var field in classElement.fields) {
        if (field.isStatic && field.isPublic && field.type == classElement.thisType) {
          List<String> segs = [];
          segs.add('MayaXmlWidget.registerInline(${classElement.name}, "${field.name}", true, (node, method) {');
  ...
  void _processCollectionType(BuilderStatus status, DartObject dartObject) {
    ...
    for (var field in classElement.fields) {
      if (field.isStatic && field.isPublic) {
        ...
        List<String> segs = [];
        segs.add('MayaXmlWidget.registerInline(${targetType.getDisplayString(withNullability: false)}, "${field.name}", true, (node, method) {');
        segs.add("return ${classElement.name}.${field.name};");
        segs.add('});');
...
MayaXmlWidgetBuilder builderFactory(BuilderOptions options) {
  return MayaXmlWidgetBuilder(options);
}